/* ===== Ball Sort Puzzle — Shop & IAP System ===== */
(function() {
  'use strict';
  ...
  function createShopPanel() {
    ...
    shopContainer.innerHTML = `
      <div class="shop-overlay"></div><div class="shop-window">
      <button class="shop-close">&times;</button><h2 class="shop-title">🛒 Ball Sort Shop</h2>
      <div class="shop-balance-bar"><span class="balance-item"><span class="coin-icon">🪙</span> <span id="shop-coins">0</span></span><span class="balance-item"><span class="gem-icon">💎</span> <span id="shop-gems">0</span></span></div>
      <div class="shop-tabs"><button class="shop-tab" data-tab="coins">🪙 Shop</button><button class="shop-tab" data-tab="gems">💎 Gems</button><button class="shop-tab" data-tab="upgrades">⚡ Upgrades</button><button class="shop-tab" data-tab="premium">👑 Premium</button></div>
      <div class="shop-content" id="shop-content"></div></div>`;
  ...
  function updateBalances() {
    const coins = shopContainer?.querySelector('#shop-coins'); const gems = shopContainer?.querySelector('#shop-gems');
    if (coins) coins.textContent = ProgressionSystem.getCoinBalance(); if (gems) gems.textContent = ProgressionSystem.getGemBalance();
  }
  ...
  function renderCoinShop(container) {
    ...
    html += '</div></div><div class="shop-section"><h3>🎲 Ball Styles</h3><div class="shop-grid">';
    for (const s of catalog.pieceStyles) {
      const owned = state.ownedPieceStyles.includes(s.id); const active = state.activePieceStyle === s.id;
      html += `<div class="shop-item ${owned ? 'owned' : ''} ${active ? 'active' : ''}" data-type="pieceStyle" data-id="${s.id}" data-price="${s.price}"><div class="item-name">${s.name}</div><div class="item-desc">${s.desc}</div>${owned ? (active ? '<span class="item-status">✓ Active</span>' : '<button class="btn-equip">Equip</button>') : `<button class="btn-buy">🪙 ${s.price}</button>`}</div>`;
    }
    ...
    for (const b of catalog.boosters) {
      html += `<div class="shop-item" data-type="booster" data-id="${b.id}" data-price="${b.price}"><div class="item-name">${b.name}</div><div class="item-desc">${b.desc}</div><button class="btn-buy">🪙 ${b.price}</button></div>`;
    }
  ...
  function handleEquip(type, id) {
    ...
    if (type === 'theme' || type === 'pieceStyle') {
      const key = type === 'theme' ? 'activeTheme' : 'activePieceStyle'; const arr = type === 'theme' ? state.ownedThemes : state.ownedPieceStyles;
      if (!arr.includes(id)) return; state[key] = id; ProgressionSystem.save(); showTab('coins'); showNotification(`${type === 'theme' ? 'Theme' : 'Style'} applied! ✅`);
    }
  }
  ...
  function renderUpgradeStation(container) {
    ...
    html += `<div class="bonus-summary"><span>⚔️ Score: <strong>${(bonuses.scoreMult).toFixed(2)}x</strong></span><span>📦 Tubes: <strong>+${bonuses.extraTube}</strong></span><span>🔄 Undo: <strong>+${bonuses.undoCount}</strong></span><span>💡 Hints: <strong>+${bonuses.hintCount}</strong></span></div>`;
    ...
    for (const [cat, tier] of Object.entries(tiers)) {
      const currentLevel = state.upgrades[cat] || 0; const currentData = tier.levels[currentLevel]; const nextData = tier.levels[currentLevel + 1]; const maxed = currentLevel >= tier.maxLevel; const costs = maxed ? null : ProgressionSystem.getUpgradeCost(cat, currentLevel);
  ...
  function renderPremiumShop(container) {
    ...
    let html = '<div class="shop-section"><h3>👑 Premium Shop</h3><p class="shop-subtitle">Exclusive items — real money only</p><h4>⚔️ Legendary Ball Skins</h4><div class="shop-grid">';
  ...
  function formatBonusName(key) { const names = { scoreMult: 'Score', undoBonus: 'Undo Bonus', extraTube: 'Extra Tube', tubeBonus: 'Tube Bonus', hintCount: 'Hints', undoCount: 'Undos' }; return names[key] || key; }
